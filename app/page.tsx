...
function Discover({ papers, saved, save, refresh, loading, selectedAreas }: { papers: Paper[]; saved: Paper[]; save: (paper: Paper) => Promise<void>; refresh: (area?: string, allToday?: boolean) => Promise<void>; loading: boolean; selectedAreas: string[] }) { const [area, setArea] = useState(selectedAreas[0] || 'math.AG'); return <div className="mx-auto max-w-5xl p-6 sm:p-10"><div className="flex flex-wrap items-end justify-between gap-3"><h2 className="text-3xl font-bold tracking-[-.055em]">Daily arXiv</h2><div className="discovery-actions"><button onClick={() => void refresh()}>{loading ? 'Loading…' : 'My areas'}</button><label><span>Category</span><select value={area} onChange={(event) => setArea(event.target.value)}>{mathAreas.map(([id, label]) => <option key={id} value={id}>{id} · {label}</option>)}</select></label><button className="discovery-all" onClick={() => void refresh(area, true)} disabled={loading}>{loading ? 'Loading…' : 'Load today'}</button></div></div><div className="mt-8 space-y-3">{papers.length ? papers.map((paper) => { const inVault = saved.some((item) => item.arxivId === paper.arxivId); return <article key={paper.arxivId} className="rounded-xl border border-[#e1e6df] bg-white p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div className="max-w-2xl"><span className="rounded bg-[#edf4ee] px-2 py-1 text-[10px] font-bold text-[#46705a]">{paper.category}</span><h3 className="mt-3 text-lg font-bold">{paper.title}</h3><p className="mt-1 text-xs text-[#737c73]">{paper.authors} · arXiv:{paper.arxivId}</p><p className="mt-3 text-sm leading-6 text-[#606a60]">{paper.abstract}</p></div><button disabled={inVault} onClick={() => void save(paper)} className={`rounded-md px-3 py-2 text-xs font-bold ${inVault ? 'bg-[#edf0ec] text-[#869086]' : 'bg-[#2d654f] text-white'}`}>{inVault ? 'Saved' : '+ Save'}</button></div></article>; }) : <div className="library-empty">No papers found.</div>}</div></div>; }

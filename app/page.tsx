...
function renderMath(expression: string, displayMode: boolean) {
  ...
  try { return katex.renderToString(normalized, { throwOnError: true, strict: 'ignore', displayMode }); }
  catch { return null; }
}
...
function Reader({ paper, audit, openImport, selectedNodeId, setSelectedNodeId, expanded, setExpanded, marks, setMark, readerNotes, notes, answers, patches, savePatches, suggestEdit, graph, analysing, askingId, analyze, askNode, saveNote, updateNote, addLink, removeLink, openUnit, profile }: ReaderProps) {
  ...
  return <section className={`reader-page ${mode === 'source' ? 'reader-original-mode' : ''} ${analysing ? 'reader-audit-locked' : ''} ${isFullscreen ? 'reader-fullscreen-active' : ''}`}><div className="fullscreen-edge-top" aria-hidden="true" /><div className="reader-titlebar"><h2>{paper.title}</h2>{analysing && <span className="reader-edit-lock"><i />Audit running · editing paused</span>}<div className="reader-top-actions">{hasOriginalPdf && <button className={mode === 'source' ? 'active reader-back-to-paper' : ''} onClick={() => mode === 'source' ? returnToEnhancedPaper() : openOriginalPaper(node?.anchor.page)} aria-label={mode === 'source' ? 'Back to enhanced paper' : 'Open original paper'}><ReaderIcon name="original" /><span>{mode === 'source' ? 'Back to paper' : 'Original paper'}</span></button>}<button className={fontPanelOpen ? 'active' : ''} onClick={() => { setFontPanelOpen(!fontPanelOpen); setPrintOpen(false); }} aria-label="Adjust paper text size"><ReaderIcon name="magnify" /><span>Text size</span><output>{Math.round(paperScale * 100)}%</output></button><button className={isFullscreen ? 'active' : ''} onClick={() => void toggleFullscreen()} aria-label={isFullscreen ? 'Exit fullscreen reading' : 'Enter fullscreen reading'}><ReaderIcon name="fullscreen" /><span>{isFullscreen ? 'Exit full screen' : 'Full screen'}</span></button><button className={printOpen ? 'active' : ''} onClick={() => { if (mode === 'source') printOriginalPaper(); else { setPrintOpen(!printOpen); setFontPanelOpen(false); } }} aria-label={mode === 'source' ? 'Print the original paper' : 'Print or save the working paper'}><ReaderIcon name="print" /><span>Print</span></button><button onClick={analyze} disabled={analysing} className="reader-reaudit">{analysing ? 'Auditing…' : 'Re-audit'}</button></div></div>{activePath && focusPath && mode !== 'source' && <div className="reader-path"><b>{activePath.goal}</b><span>{activePath.reason}</span><button onClick={() => setFocusPath(false)}>Show full paper</button></div>}
    <div className={`reader-grid ${outlineOpen ? 'reader-grid-outline' : ''} ${inspectorOpen ? 'reader-grid-inspector' : ''}`}>{outlineOpen && <aside className="reader-outline reader-drawer" data-reader-floating-panel><div className="drawer-head"><div><span className="reader-kicker">Logical outline</span><b>{units.length} units</b></div><button onClick={() => setOutlineOpen(false)} aria-label="Close outline">×</button></div><div className="space-y-0.5">{units.map((item) => { const changed = patchForNode(patches, item.id); return <button key={item.id} onClick={() => jumpToDocumentUnit(item.id)} className={`outline-unit ${selectedNodeId === item.id ? 'outline-unit-active' : ''}`}><span className={`outline-kind ${kindClass(item.kind)}`}>{item.kind[0].toUpperCase()}</span><span className="min-w-0"><small>{displayUnitLabel(item)}</small><b><MathText value={item.title} /></b></span>{changed && <i className="working-dot" title={changed.kind === 'add' ? 'Added in working edition' : 'Edited in working edition'}>W</i>}{item.status !== 'verified' && !changed && <i className="ml-auto h-1.5 w-1.5 flex-none rounded-full bg-[#b8873c]" />}</button>; })}</div></aside>}
      ...
      {inspectorOpen && mode !== 'source' && <aside className="reader-inspector reader-drawer" data-reader-floating-panel style={assistantSize ? { width: assistantSize.width, height: assistantSize.height } : undefined}><div className="drawer-head"><b>Assistant</b><button onClick={() => setInspectorOpen(false)} aria-label="Close assistant">×</button></div>{node ? <NodeInspector key={`${edition}:${node.id}`} paper={paper} node={node} originalNode={audit.nodes.find((item) => item.id === node.id)} plainSource={node.id.startsWith('source-block:')} edition={edition} patches={patches} savePatches={savePatches} suggestEdit={suggestEdit} expanded={expanded[node.id] !== false} setExpanded={(value) => setExpanded(node.id, value)} expandProof={expandProofRequest} notes={notes.filter((item) => item.nodeId === node.id).slice(0, 1)} answer={answers[node.id]} question={question} setQuestion={setQuestion} asking={askingId === node.id} ask={() => void askNode(node, question)} saveNote={saveNote} updateNote={updateNote} graph={graph} addLink={addLink} removeLink={removeLink} openUnit={openUnit} openOriginalPaper={openOriginalPaper} assistantRequest={assistantRequest} clearAssistantRequest={() => setAssistantRequest(null)} /> : <p className="p-4 text-xs text-[#6e6a64]">Select a document unit.</p>}<button className="assistant-resize-handle" onPointerDown={beginAssistantResize} aria-label="Resize assistant panel" title="Drag to resize" /></aside>}</div>
...
const processHistoryKey = 'proofroom:ai-activity:v1';
...
function ProcessTray({ analysis, profile, bridge }: { analysis: AnalysisProgress | null; profile: Profile; bridge: Bridge | null }) {
  const [entries, setEntries] = useState<ReaderProcessEntry[]>([]); const [historyReady, setHistoryReady] = useState(false); const [collapsed, setCollapsed] = useState(true); const [now, setNow] = useState(0);
  useEffect(() => {
    let restored: ReaderProcessEntry[] = [];
    try {
      const saved = JSON.parse(window.localStorage.getItem(processHistoryKey) ?? '[]') as ReaderProcessEntry[];
      if (Array.isArray(saved)) restored = saved.slice(0, 14).map((item) => item.status === 'running' ? { ...item, status: 'error', detail: `${item.detail} · Interrupted when the reader closed`, updatedAt: Date.now() } : item);
    } catch { /* Start with an empty local history. */ }
    const frame = window.requestAnimationFrame(() => { setEntries(restored); setHistoryReady(true); });
    return () => window.cancelAnimationFrame(frame);
  }, []);
  useEffect(() => {
  ...
  useEffect(() => { if (historyReady) window.localStorage.setItem(processHistoryKey, JSON.stringify(entries.slice(0, 14))); }, [entries, historyReady]);
  ...
  return <aside className="process-tray" aria-live="polite"><header><ProcessMascot busy={running > 0} /><div><b>AI activity</b><span>{running ? `${running} running` : 'All finished'}</span></div><button onClick={() => setCollapsed(true)} aria-label="Collapse activity tray">⌄</button></header><div>{visibleEntries.length ? visibleEntries.map((item) => <article key={item.id} className={`process-entry process-${item.status}`} data-detail={item.detail} tabIndex={0}><i>{item.status === 'complete' ? '✓' : item.status === 'error' ? '!' : ''}</i><div><b>{item.label}</b><span>{item.detail}</span></div><time>{item.status === 'running' ? `${Math.max(0, Math.floor((now - item.startedAt) / 1000))}s` : item.status === 'complete' ? 'Done' : 'Stopped'}</time></article>) : <p className="process-empty">No activity.</p>}</div></aside>;
}

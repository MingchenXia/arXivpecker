...
type ReaderProcessTarget = { paperId: string; nodeId?: string; panel: 'assistant' | 'paper-chat' };
type ReaderProcessUpdate = { id: string; label: string; detail: string; status: ReaderProcessStatus; retryPaperId?: string; resultTarget?: ReaderProcessTarget };
type ReaderNavigationRequest = ReaderProcessTarget & { nonce: number };
...
function Reader({ paper, audit, openImport, selectedNodeId, setSelectedNodeId, expanded, setExpanded, marks, setMark, readerNotes, notes, answers, savePaperMessages, patches, savePatches, suggestEdit, graph, analysing, auditActionLabel, askingId, analyze, askNode, rememberAuditThread, saveNote, updateNote, deleteNote, addLink, removeLink, openUnit, profile, navigationRequest }: ReaderProps) {
  ...
  useEffect(() => {
    ...
    const openFrame = window.requestAnimationFrame(() => {
      ...
      setSelectedNodeId(navigationRequest.nodeId); setAssistantRequest((current) => ({ view: 'ask', nonce: (current?.nonce ?? 0) + 1 }));
      jumpFrame = window.requestAnimationFrame(() => window.dispatchEvent(new CustomEvent('proofroom:jump-unit', { detail: navigationRequest.nodeId })));
    });
    return () => { window.cancelAnimationFrame(openFrame); if (jumpFrame) window.cancelAnimationFrame(jumpFrame); };
  ...
  async function askPaperContext() {
    ...
    const processId = `paper-question:${paper.id}`; const resultTarget: ReaderProcessTarget = { paperId: paper.id, panel: 'paper-chat' };
    reportReaderProcess({ id: processId, label: 'Full-paper question', detail: paper.title, status: 'running', resultTarget });
    ...
    try {
      ...
      const completed: PaperChatMessage[] = [...paperMessagesRef.current, { role: 'assistant', text: readString(data.text) }]; paperMessagesRef.current = completed; setPaperMessages(completed); savePaperMessages(completed); reportReaderProcess({ id: processId, label: 'Full-paper answer ready', detail: paper.title, status: 'complete', resultTarget });
...
function InteractiveDocumentComponent({ paper, audit, nodes, notes, saveNote, updateNote, deleteNote, setSelectedNodeId, expanded, setExpanded, marks, setMark, patches, savePatches, openAssistant, openReference, expandCitation, attachCitation, expandProofStep, expandProofRequest }: InteractiveDocumentProps) {
  ...
  useEffect(() => {
    const jump = (event: Event) => {
      ...
      viewportSelectionRef.current = targetId; selectionLockRef.current = { nodeId: targetId, until: Date.now() + 1400 };
      ...
      window.requestAnimationFrame(() => window.requestAnimationFrame(() => {
        ...
        document.querySelector<HTMLElement>(`.reader-document [data-node-id="${CSS.escape(targetId)}"]`)?.scrollIntoView({ behavior: 'smooth', block: 'center' });
      }));
    };
...
function ProcessTray({ openResult, retryAudit }: { openResult: (target: ReaderProcessTarget) => void; retryAudit: (paperId: string) => void }) {
  ...
  function activateResult(item: ReaderProcessEntry) { if (item.status !== 'complete' || !item.resultTarget) return; openResult(item.resultTarget); setCollapsed(true); }
  ...
  return <aside className="process-tray" aria-live="polite"><header><ProcessMascot busy={running > 0} /><div><b>AI activity</b><span>{running ? `${running} running` : 'All finished'}</span></div><button onClick={() => setCollapsed(true)} aria-label="Collapse activity tray">⌄</button></header><div>{visibleEntries.length ? visibleEntries.map((item) => {
    const navigable = item.status === 'complete' && Boolean(item.resultTarget);
    return <article key={item.id} className={`process-entry process-${item.status} ${navigable ? 'process-navigable' : ''}`} data-detail={item.detail} role={navigable ? 'button' : undefined} tabIndex={navigable ? 0 : undefined} aria-label={navigable ? `Open ${item.label}: ${item.detail}` : undefined} title={navigable ? 'Open result' : undefined} onClick={() => activateResult(item)} onKeyDown={(event) => { if (!navigable || (event.key !== 'Enter' && event.key !== ' ')) return; event.preventDefault(); activateResult(item); }}><i>{item.status === 'complete' ? '✓' : item.status === 'error' ? '!' : ''}</i><div><b>{item.label}</b><span>{item.detail}</span></div><div className="process-entry-tail"><time>{item.status === 'running' ? elapsedLabel(item.startedAt, now) : item.status === 'complete' ? 'Done' : 'Stopped'}</time>{navigable && <span className="process-entry-open" aria-hidden="true">→</span>}{item.status === 'error' && item.retryPaperId && <button onClick={() => retryAudit(item.retryPaperId as string)} aria-label="Retry AI audit" title="Retry audit">↻</button>}</div></article>;
  }) : <p className="process-empty">No activity.</p>}</div></aside>;
}

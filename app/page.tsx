...
function UnitStatusRail({ noteCount, openNote, patch, originalValue, currentValue, citations, revert }: { noteCount: number; openNote: () => void; patch?: WorkingPatch; originalValue: string; currentValue: string; citations: CitationReference[]; revert: () => Promise<void> }) {
  ...
  return <div className="unit-status-rail"><div>{noteCount > 0 && <NoteMarker count={noteCount} open={openNote} />}{modified && patch && <ModifiedMarker patch={patch} originalValue={originalValue} currentValue={currentValue} citations={citations} revert={revert} />}</div></div>;
}

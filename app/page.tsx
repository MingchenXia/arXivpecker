...
function Reader({ paper, audit, openImport, selectedNodeId, setSelectedNodeId, expanded, setExpanded, marks, setMark, readerNotes, setReaderNote, notes, answers, patches, savePatches, suggestEdit, graph, analysing, askingId, analyze, askNode, saveNote, addLink, removeLink, openUnit, profile }: ReaderProps) {
  ...
  useEffect(() => { const saved = Number(window.localStorage.getItem(paperScaleKey)); if (Number.isFinite(saved) && saved >= .8 && saved <= 1.4) setPaperScale(saved); }, []);
  ...
  return <section className="reader-page"><div className="fullscreen-edge-top" aria-hidden="true" /><div className="reader-titlebar"><h2>{paper.title}</h2><div className="reader-top-actions"><button className={mode === 'source' ? 'active' : ''} onClick={() => { const showingOriginal = mode === 'source'; setMode(showingOriginal ? 'interactive' : 'source'); setEdition(showingOriginal ? 'working' : 'original'); }} aria-label={mode === 'source' ? 'Return to enhanced paper' : 'Open original paper'}><ReaderIcon name="original" /><span>{mode === 'source' ? 'Enhanced paper' : 'Original paper'}</span></button><button className={fontPanelOpen ? 'active' : ''} onClick={() => { setFontPanelOpen(!fontPanelOpen); setPrintOpen(false); }} aria-label="Adjust paper text size"><ReaderIcon name="magnify" /><span>Text size</span><output>{Math.round(paperScale * 100)}%</output></button><button className={isFullscreen ? 'active' : ''} onClick={() => void toggleFullscreen()} aria-label={isFullscreen ? 'Exit fullscreen reading' : 'Enter fullscreen reading'}><ReaderIcon name="fullscreen" /><span>{isFullscreen ? 'Exit full screen' : 'Full screen'}</span></button><button className={printOpen ? 'active' : ''} onClick={() => { setPrintOpen(!printOpen); setFontPanelOpen(false); }} aria-label="Print or save a paper PDF"><ReaderIcon name="print" /><span>Print</span></button><button onClick={analyze} disabled={analysing} className="reader-reaudit">{analysing ? 'Auditing…' : 'Re-audit'}</button></div></div>{activePath && focusPath && <div className="reader-path"><b>{activePath.goal}</b><span>{activePath.reason}</span><button onClick={() => setFocusPath(false)}>Show full paper</button></div>}
    ...
    {paperChatOpen && <PaperChatDialog paper={paper} currentNode={node} messages={paperMessages} question={paperQuestion} setQuestion={setPaperQuestion} asking={paperAsking} ask={() => void askPaperContext()} close={() => setPaperChatOpen(false)} />}{printOpen && <PrintPanel paper={paper} nodes={editionNodes} edition={edition} setEdition={setEdition} mode={mode} setMode={setMode} readerNotes={readerNotes} notes={notes} close={() => setPrintOpen(false)} />}{referenceOpen && <ReferenceReaderPanel paper={paper} graph={graph} target={referenceTarget} setTarget={setReferenceTarget} close={() => setReferenceOpen(false)} />}{exportOpen && <ExportPaperPanel paper={paper} audit={audit} nodes={editionNodes} focusIds={activePath?.nodeIds ?? []} readerNotes={readerNotes} notes={notes} focusActive={focusPath} close={() => setExportOpen(false)} />}{comparisonOpen && <VersionComparisonPanel paper={paper} profile={profile} audit={audit} openUnit={openUnit} close={() => setComparisonOpen(false)} />}</section>;
}

function PrintPanel({ paper, nodes, edition, setEdition, mode, setMode, readerNotes, notes, close }: { paper: Paper; nodes: AuditNode[]; edition: EditionMode; setEdition: (value: EditionMode) => void; mode: ReaderMode; setMode: (value: ReaderMode) => void; readerNotes: Record<string, string>; notes: Note[]; close: () => void }) {
  const [printEdition, setPrintEdition] = useState<EditionMode>(edition);
  ...
  async function buildPrintView() {
    ...
    setMode('interactive'); setEdition(printEdition);
    ...
    clone.querySelectorAll('.tex-edit-hint,.reading-mark-control,.proof-reading-tools,.audit-peek,.source-result > footer,.paper-reading-guide,.source-completeness').forEach((element) => element.remove());
  ...
  return <aside className="print-panel reader-floating-panel" aria-label="Print options"><header><div><b>Print or save as PDF</b><span>Builds a clean paper locally</span></div><button onClick={close} aria-label="Close print options">×</button></header><div className="print-options"><label><span>Edition</span><select value={printEdition} onChange={(event) => setPrintEdition(event.target.value as EditionMode)}><option value="working">Working edition</option><option value="original">Author edition</option></select></label><label><input type="checkbox" checked={annotations} onChange={(event) => setAnnotations(event.target.checked)} /><span>Reading marks and edit highlights</span></label><label><input type="checkbox" checked={includeNotes} onChange={(event) => setIncludeNotes(event.target.checked)} /><span>Reader notes</span></label><label><input type="checkbox" checked={citations} onChange={(event) => setCitations(event.target.checked)} /><span>Cited-source cards</span></label><label><input type="checkbox" checked={proofMaps} onChange={(event) => setProofMaps(event.target.checked)} /><span>AI proof maps</span></label></div>{error && <p className="panel-error">{error}</p>}<footer><span>The browser print dialog can print or save the compiled view as PDF.</span><button onClick={() => void buildPrintView()}>Build PDF & print</button></footer></aside>;
}

function ReferenceReaderPanel({ paper, graph, target, setTarget, close }: { paper: Paper; graph: Graph; target: ReferenceTarget | null; setTarget: (value: ReferenceTarget | null) => void; close: () => void }) {
...
function NodeInspector({ paper, node, originalNode, edition, patches, savePatches, suggestEdit, expanded, setExpanded, expandProof, readerNote, setReaderNote, answer, question, setQuestion, asking, ask, saveNote, graph, addLink, removeLink, openUnit }: InspectorProps) {
  const [noteText, setNoteText] = useState(''); const [noteLatex, setNoteLatex] = useState(''); const [target, setTarget] = useState(''); const [relation, setRelation] = useState<CrossLink['relation']>('uses'); const [linkNote, setLinkNote] = useState(''); const [advancedOpen, setAdvancedOpen] = useState(false); const [toolView, setToolView] = useState<'edit' | 'notes' | 'context'>('edit');
...
function ImportDialog({ close, importArxiv, profile, setProfile, bridge }: { close: () => void; importArxiv: (value: string, convertPdfToLatex?: boolean, correctnessAudit?: boolean, detailedAudit?: boolean) => Promise<void>; profile: Profile; setProfile: (value: Profile | ((old: Profile) => Profile)) => void; bridge: Bridge | null }) {
  const [value, setValue] = useState(''); const [convertPdf, setConvertPdf] = useState(true); const [correctnessAudit, setCorrectnessAudit] = useState(true); const [detailedAudit, setDetailedAudit] = useState(true); const [busy, setBusy] = useState(false); const [error, setError] = useState('');
  async function submit(event: FormEvent) { event.preventDefault(); if (!value.trim()) return; setBusy(true); setError(''); try { await importArxiv(value, convertPdf, correctnessAudit, detailedAudit); } catch (cause) { setError(cause instanceof Error ? cause.message : 'The paper could not be imported.'); } finally { setBusy(false); } }
  return <div className="import-overlay" onMouseDown={(event) => { if (!busy && event.currentTarget === event.target) close(); }}><form onSubmit={submit} className="import-dialog"><header><div><p className="reader-kicker">ArXiv to interactive paper</p><h2>Import & audit a paper</h2></div><button type="button" onClick={close} disabled={busy}>×</button></header><p className="import-explainer">Paste an arXiv ID, abstract URL, or PDF URL. Proofroom fetches TeX first, reads the complete paper with your local Codex subscription, then builds a theorem-level dependency map.</p><label className="import-source"><span>arXiv ID or URL</span><input autoFocus value={value} onChange={(event) => setValue(event.target.value)} placeholder="https://arxiv.org/abs/2608.24719" /></label><div className="import-options"><label className="import-depth"><span><b>Audit depth</b><small>Detailed mode follows theorem-level citations into their primary sources.</small></span><select value={detailedAudit ? 'detailed' : 'standard'} onChange={(event) => setDetailedAudit(event.target.value === 'detailed')}><option value="detailed">Detailed</option><option value="standard">Standard</option></select></label><label className="import-convert"><input type="checkbox" checked={convertPdf} onChange={(event) => setConvertPdf(event.target.checked)} /><span><b>Convert PDF-only papers to LaTeX first</b><small>If arXiv has no usable TeX source, local AI creates a saved, editable LaTeX working source before the audit.</small></span></label><label className="import-convert"><input type="checkbox" checked={correctnessAudit} onChange={(event) => setCorrectnessAudit(event.target.checked)} /><span><b>Audit mathematical correctness</b><small>Check each statement and proof against its assumptions and dependencies. Every formal environment receives a compact status you can inspect in place.</small></span></label></div><section className="import-ai"><div><b>AI for this audit</b><span>{bridge?.account ? `Local ${bridge.account.type}${bridge.account.planType ? ` · ${bridge.account.planType}` : ''}` : 'Start the local Codex bridge first'}</span></div><ModelControls profile={profile} setProfile={setProfile} bridge={bridge} /></section>{error && <p className="import-error">{error}</p>}<footer><button type="button" onClick={close} disabled={busy}>Cancel</button><button className="import-submit" disabled={busy || !value.trim() || !bridge?.account}>{busy ? <><i /> Fetching metadata…</> : 'Import & analyze'}</button></footer></form></div>;
}

...
function MathText({ value, block = false, citations = [], explicitOnly = false }: { value: string; block?: boolean; citations?: CitationReference[]; explicitOnly?: boolean }) {
  const parts = useMemo(() => {
    ...
    for (const match of source.matchAll(pattern)) {
      ...
      if (index > cursor) result.push({ text: source.slice(cursor, index).replace(/\uE000/g, '$'), math: false, display: false });
      const token = match[0];
...
function RailIcon({ name }: { name: 'reader' | 'library' | 'discover' | 'settings' }) {
  const paths = { reader: <><path d="M5 4.5h14v15H5z" /><path d="M8 8h8M8 12h8M8 16h5" /></>, library: <><path d="M3.5 5.5c3-1 5.8-.5 8.5 1.3v13c-2.7-1.8-5.5-2.3-8.5-1.3z" /><path d="M20.5 5.5c-3-1-5.8-.5-8.5 1.3v13c2.7-1.8 5.5-2.3 8.5-1.3z" /></>, discover: <><path d="m12 3 1.8 5.2L19 10l-5.2 1.8L12 17l-1.8-5.2L5 10l5.2-1.8z" /><path d="m19 16 .7 2.3L22 19l-2.3.7L19 22l-.7-2.3L16 19l2.3-.7z" /></>, settings: <><path d="M12.2 3h-.4a1.8 1.8 0 0 0-1.8 1.8v.3a1.8 1.8 0 0 1-.9 1.55l-.35.2a1.8 1.8 0 0 1-1.8 0l-.25-.14a1.8 1.8 0 0 0-2.46.66l-.2.35a1.8 1.8 0 0 0 .66 2.46l.25.14a1.8 1.8 0 0 1 .9 1.56v.4a1.8 1.8 0 0 1-.9 1.56l-.25.14a1.8 1.8 0 0 0-.66 2.46l.2.35a1.8 1.8 0 0 0 2.46.66l.25-.14a1.8 1.8 0 0 1 1.8 0l.35.2a1.8 1.8 0 0 1 .9 1.55v.3a1.8 1.8 0 0 0 1.8 1.8h.4a1.8 1.8 0 0 0 1.8-1.8v-.3a1.8 1.8 0 0 1 .9-1.55l.35-.2a1.8 1.8 0 0 1 1.8 0l.25.14a1.8 1.8 0 0 0 2.46-.66l.2-.35a1.8 1.8 0 0 0-.66-2.46l-.25-.14a1.8 1.8 0 0 1-.9-1.56v-.4a1.8 1.8 0 0 1 .9-1.56l.25-.14a1.8 1.8 0 0 0 .66-2.46l-.2-.35a1.8 1.8 0 0 0-2.46-.66l-.25.14a1.8 1.8 0 0 1-1.8 0l-.35-.2a1.8 1.8 0 0 1-.9-1.55v-.3A1.8 1.8 0 0 0 12.2 3Z" /><circle cx="12" cy="12" r="2.6" /></> }[name];
...
export default function Home() {
  ...
  async function analyzePaper(target: Paper, convertPdfToLatex = false, correctnessAudit = true, detailedAudit = true, requestedProcessId?: string) {
    ...
    reportReaderProcess({ id: processId, label: 'AI audit in progress', detail: target.title, status: 'running' });
    try {
    ...
    } catch (error) { const message = error instanceof Error ? error.message : 'Analysis failed.'; reportReaderProcess({ id: processId, label: 'AI audit stopped', detail: message, status: 'error', retryPaperId: target.id }); notify(message.includes('wait limit') ? `${message} The paper and TeX source are preserved; retry from the library with a faster model or lower effort.` : `${message} Check the local Codex connection and try again.`); void refreshBridge(); }
...
function Reader({ paper, audit, openImport, selectedNodeId, setSelectedNodeId, expanded, setExpanded, marks, setMark, readerNotes, notes, answers, patches, savePatches, suggestEdit, graph, analysing, askingId, analyze, askNode, saveNote, updateNote, deleteNote, addLink, removeLink, openUnit, profile }: ReaderProps) {
  ...
  const hasOriginalPdf = !paper.arxivId.startsWith('local-');
  if (!audit) return <section className="mx-auto max-w-3xl px-6 py-14"><div className="rounded-xl border border-[#d8e4d9] bg-[#f3f8f3] p-7"><h2 className="text-2xl font-bold tracking-[-.04em]">Analyze this paper to begin.</h2><div className="mt-6 flex gap-2"><button onClick={analyze} disabled={analysing} className="rounded-md bg-[#2d654f] px-4 py-2.5 text-xs font-bold text-white disabled:opacity-50">{analysing ? 'Auditing…' : 'Analyze'}</button>{hasOriginalPdf && <a href={`https://arxiv.org/pdf/${paper.arxivId}`} target="_blank" rel="noreferrer" className="rounded-md border border-[#cddbd0] bg-white px-4 py-2.5 text-xs font-bold text-[#39634c]">Original PDF ↗</a>}</div></div></section>;
  ...
  const activePath = selectedTarget ? { goal: `Focus on ${displayUnitLabel(selectedTarget)}`, nodeIds: dependencyFocus(editionNodes, selectedTarget.id), reason: 'Showing only this result and the parts it depends on; unrelated branches are hidden.' } : audit.readingPaths[selectedPathIndex] ?? audit.readingPaths[0];
...
function InteractiveDocument({ paper, audit, nodes, notes, saveNote, updateNote, deleteNote, selectedNodeId, setSelectedNodeId, expanded, setExpanded, marks, setMark, patches, savePatches, openAssistant, openReference, expandCitation, attachCitation, expandProofStep, expandProofRequest }: { paper: Paper; audit: PaperAudit; nodes: AuditNode[]; notes: Note[]; saveNote: (anchor: string, nodeId: string, text: string, latex: string) => void; updateNote: (noteId: string, text: string) => void; deleteNote: (noteId: string) => void; selectedNodeId: string; setSelectedNodeId: (id: string) => void; expanded: Record<string, boolean>; setExpanded: (id: string, value: boolean) => void; marks: Record<string, Exclude<ReadingMark, ''>>; setMark: (id: string, value: ReadingMark) => void; patches: WorkingPatch[]; savePatches: (patches: WorkingPatch[]) => Promise<void>; openAssistant: (view?: 'ask' | 'notes' | 'compose-note') => void; openReference: (citation?: CitationReference) => void; expandCitation: (node: AuditNode, citation: CitationReference) => void; attachCitation: (citation: CitationReference, file: File) => Promise<string>; expandProofStep: (node: AuditNode, step: string, index: number) => Promise<string>; expandProofRequest: (node: AuditNode, request: string) => Promise<string> }) {
  ...
  return <article className="interactive-document source-document">
    <header className="interactive-lead">
      <h1>{paper.title}</h1><div className="paper-metadata"><p className="paper-authors">{paper.authors}</p><p className="paper-identity">{paperSourceLabel(paper)}{paper.arxivId.startsWith('local-') ? '' : ` · ${paper.category}`}</p></div>
...
function Discover({ papers, saved, save, refresh, loading, selectedAreas }: { papers: Paper[]; saved: Paper[]; save: (paper: Paper) => Promise<void>; refresh: (area?: string, latestBatch?: boolean) => Promise<void>; loading: boolean; selectedAreas: string[] }) { const [area, setArea] = useState(selectedAreas[0] || 'math.AG'); return <div className="mx-auto max-w-5xl p-6 sm:p-10"><div className="flex flex-wrap items-end justify-between gap-3"><h2 className="text-3xl font-bold tracking-[-.055em]">Latest arXiv</h2><div className="discovery-actions"><button onClick={() => void refresh()}>{loading ? 'Loading…' : 'My areas'}</button><label><span>Category</span><select value={area} onChange={(event) => setArea(event.target.value)}>{mathAreas.map(([id, label]) => <option key={id} value={id}>{id} · {label}</option>)}</select></label><button className="discovery-all" onClick={() => void refresh(area, true)} disabled={loading}>{loading ? 'Loading…' : 'Load latest'}</button></div></div><div className="mt-8 space-y-3">{papers.length ? papers.map((paper) => { const inVault = saved.some((item) => item.arxivId.replace(/v\d+$/i, '') === paper.arxivId.replace(/v\d+$/i, '')); return <article key={paper.arxivId} className="rounded-xl border border-[#e1e6df] bg-white p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div className="max-w-2xl"><span className="rounded bg-[#edf4ee] px-2 py-1 text-[10px] font-bold text-[#46705a]">{paper.category}</span><h3 className="mt-3 text-lg font-bold">{paper.title}</h3><p className="mt-1 text-xs text-[#737c73]">{paper.authors} · arXiv:{paper.arxivId}</p>{paper.abstract && <p className="mt-3 text-sm leading-6 text-[#606a60]">{paper.abstract}</p>}</div><button disabled={inVault} onClick={() => void save(paper)} className={`rounded-md px-3 py-2 text-xs font-bold ${inVault ? 'bg-[#edf0ec] text-[#869086]' : 'bg-[#2d654f] text-white'}`}>{inVault ? 'Saved' : '+ Save'}</button></div></article>; }) : <div className="library-empty">No papers found.</div>}</div></div>; }
